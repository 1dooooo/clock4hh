...
class Calendar:
  def __init__(s, point = None):
    ...
    sel_bg = '#ecffc4'
    sel_fg = '#05640e'
    s._date = datetime(year, month, 1, hour, minute)        #每月第一日
  ...
  def __place_widgets(s):
    ...
    s.CB_hour = ttk.Combobox(
        ...
        # font=("", 20),  # 字体
        values=['%02d' % hour for hour in range(0,24)],  # 设置下拉框的选项
    )
    s.CB_hour.current(datetime.now().hour)
    s.CB_hour.bind("<<ComboboxSelected>>", s._update)
    s.CB_hour.grid(in_=hframe, column=1, row=1)
    ttk.Label(hframe, text="时").grid(in_=hframe, column=2, row=1)
 
    s.CB_minute = ttk.Combobox(
        master=hframe,  # 父容器
        height=15,  # 高度,下拉显示的条目数量
        width=2,  # 宽度
        state="normal",  # 设置状态 normal(可选可输入)、readonly(只可选)、 disabled
        cursor="arrow",  # 鼠标移动时样式 arrow, circle, cross, plus...
        # font=("", 20),  # 字体
        values=['%02d' % minute for minute in range(0,60)],  # 设置下拉框的选项
    )
    s.CB_minute.current(datetime.now().minute)
    s.CB_minute.bind("<<ComboboxSelected>>", s._update)
    s.CB_minute.grid(in_=hframe, column=3, row=1)
    ttk.Label(hframe, text="分").grid(in_=hframe, column=4, row=1)
  ...
  def _prev_month(s):
    ...
    s._date = datetime(s._date.year, s._date.month, 1, s._date.hour, s._date.minute)
    s.CB_year.set(s._date.year)
    s.CB_month.set(s._date.month)
    s.CB_hour.set(s._date.hour)
    s.CB_minute.set(s._date.minute)
    s._update()
  ...
  def _update(s, event = None, key = None):
    ...
    minute = int(s.CB_minute.get())
    
    if year == 0 or year > 9999: return
    s._canvas.place_forget()
    s._date = datetime(year, month, 1,hour,minute)
  ...
  def selection(s):
    ...
    year, month, hour, minute = s._date.year, s._date.month,s._date.hour, s._date.minute
    # return str(datetime(year, month, int(s._selection[0])))[:10]
    return str(datetime(year, month, int(s._selection[0]), hour, minute))[:19]
...
class datepicker:
  def __init__(s,window,axes):  #窗口对象 坐标
    ...
    s.bt2.grid(row=1,column=0)
    s.ent2=tk.Entry(s.frame,textvariable=s.end_date)
    s.ent2.grid(row=1,column=1)
